...
const formatDate = (dateString: string) => {
    // The API returns formattedDate, but if we need to format the raw ISO date:
...
const TypeIcon: React.FC<{ type: string }> = ({ type }) => {
    // API returns "Outcome" for withdrawals usually
...
const WithdrawalsPage: React.FC = () => {
    ...
    const fetchWithdrawals = async () => {
        ...
        try {
            ...
            // Map formattedDate/date to createdAt if needed by backend default, otherwise use key
            ...
            // Search - pass empty string if no search
            ...
            const response = await interceptedFetch(`${API_BASE_URL}/api/v1/reports/withdrawals?${params.toString()}`);
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await interceptedFetch(`${API_BASE_URL}/api/v1/reports/withdrawals/download?${params.toString()}`);
            ...
            a.href = url;
            a.download = `Withdrawals_Report_${new Date().toISOString().split('T')[0]}.pdf`; 
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Withdrawals Report</h1>
                    <p className="text-gray-500 mt-2">A detailed report of all your withdrawal activities.</p>
                </div>
