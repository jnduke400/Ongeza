...
const GoalTransactionDetailsPage = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (!(event.target as HTMLElement).closest('.menu-container')) setOpenMenuId(null);
        };
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (sortConfig.key === 'date') apiSortKey = 'transactionDate' as any;
                if (sortConfig.key === 'source') apiSortKey = 'channel' as any;
                if (sortConfig.key === 'type') apiSortKey = 'transactionType' as any;
                const params = new URLSearchParams({
                    referenceId: goalId,
                    referenceType: 'GOAL',
                    page: String(currentPage - 1),
                    size: String(itemsPerPage),
                    sortBy: apiSortKey as string,
                ...
                const activitiesRes = await interceptedFetch(`${API_BASE_URL}/api/v1/activities?${params.toString()}`);
                const activitiesData = await activitiesRes.json();
                if (activitiesData.success) {
                    const mappedTransactions: Transaction[] = activitiesData.data.content.map((item: any) => ({
                        id: item.id,
                        transactionId: item.transactionId,
                        date: item.transactionDate,
                        description: item.description,
                        amount: item.amount,
                        type: item.transactionType === 'INCOME' ? 'Income' : 'Outcome',
                        source: item.channel,
                        status: item.status.charAt(0).toUpperCase() + item.status.slice(1).toLowerCase(),
                        paymentType: item.paymentType
                    }));
                    setTransactions(mappedTransactions);
                    setTotalPages(activitiesData.data.totalPages);
                    setTotalElements(activitiesData.data.totalElements);
                }
            } catch (e: any) {
        ...
        fetchData();
    }, [goalId, currentPage, itemsPerPage, sortConfig, activeTab, debouncedSearchQuery, dateFilter]);
    ...
    const currentIds = useMemo(() => transactions.map(tx => tx.id), [transactions]);
    const areAllOnPageSelected = useMemo(() => currentIds.length > 0 && currentIds.every(id => selectedRows.includes(id)), [currentIds, selectedRows]);
    const areSomeOnPageSelected = useMemo(() => currentIds.some(id => selectedRows.includes(id)), [currentIds, selectedRows]);
    
    useEffect(() => {
        if (headerCheckboxRef.current) headerCheckboxRef.current.indeterminate = areSomeOnPageSelected && !areAllOnPageSelected;
    }, [areSomeOnPageSelected, areAllOnPageSelected]);

    const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) setSelectedRows(Array.from(new Set([...selectedRows, ...currentIds])));
        else setSelectedRows(selectedRows.filter(id => !currentIds.includes(id)));
    };
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const params = new URLSearchParams();
            params.append('referenceId', goalId);
            params.append('referenceType', 'GOAL');
            params.append('search', searchQuery || '');
            let apiSortBy = sortConfig.key as string;
            if (apiSortBy === 'date') apiSortBy = 'createdAt';
            if (apiSortBy === 'source') apiSortBy = 'channel';
            ...
            a.download = `Transactions_${cleanGoalName}_${dateStr}.pdf`;
    ...
    return (
        <>
            ...
            <div className="space-y-6">
                <Link to={`/goals/${goalId}`} className="flex items-center text-primary hover:underline font-semibold">
                 ...
                 <div className="p-4 sm:p-6 rounded-2xl bg-surface shadow-sm text-on-surface">
                    ...
                    <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-4">
                            <div>
                                ...
                                <p className="text-gray-500 mt-1">Showing history for: <span className="font-semibold">{goalName || 'Goal'}</span></p>
                            </div>
                    ...
                    <div className="flex items-center space-x-1 sm:space-x-4 border-b border-gray-200 mb-6 overflow-x-auto">
                        {tabs.map(tab => (
                            <button
                                key={tab}
                                onClick={() => handleTabClick(tab)}
                                className={`px-2 py-3 font-semibold transition-colors duration-200 text-sm sm:text-base whitespace-nowrap ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-gray-500 hover:text-primary'}`}
                            >
                                {tab}
                            </button>
                        ))}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full min-w-[1000px] text-left text-sm">
                            <thead>
                                <tr className="border-b border-gray-200">
                                    ...
                                    <th className="p-4 font-semibold"><SortableTableHeader label="Channel" columnKey="source" sortConfig={sortConfig} requestSort={requestSort} /></th>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr key={tx.id} className="border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="p-4"><SourceCell name={tx.source} /></td>
                        ...
                        {!loading && transactions.length === 0 && <div className="text-center py-10 text-gray-500"><p>No transactions found for this goal.</p></div>}
                    </div>
...
export default GoalTransactionDetailsPage;
