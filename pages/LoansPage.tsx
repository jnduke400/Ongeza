...
const StatusPill: React.FC<{ status: Loan['status'] }> = ({ status }) => {
    const styles: { [key in Loan['status']]: string } = {
...
const LoansPage: React.FC = () => {
    ...
    const processedLoans = useMemo(() => {
        ...
        return filtered;
    }, [loans, sortConfig, searchQuery, dateFilter]);
    ...
    const handleDownloadPDF = () => {
        const doc = new jsPDF();
        const tableColumn = ["Date", "Transaction No.", "Name", "Account", "Interest Period", "Interest (%)", "Amount", "Status", "Deadline"];
    ...
    const handleExportCSV = () => {
        const headers = ["Date", "Transaction No.", "Name", "Account", "Interest Period", "Interest Rate", "Amount", "Currency", "Status", "Deadline"];
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                <h1 className="text-2xl font-bold text-gray-800">Loans Report</h1>
                <p className="text-gray-500 mt-1">This report shows all disbursed loans, their status, and repayment details.</p>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="border-b-2 border-gray-100">
                             <tr>
                                <SortableTableHeader label="Date" columnKey="date" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableTableHeader label="Transaction No." columnKey="transactionNo" sortConfig={sortConfig} requestSort={requestSort} />
                                ...
                                <SortableTableHeader label="Interest Period" columnKey="interestPeriod" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableTableHeader label="Interest (%)" columnKey="interestRate" sortConfig={sortConfig} requestSort={requestSort} />
                                ...
                                <SortableTableHeader label="Deadline" columnKey="deadline" sortConfig={sortConfig} requestSort={requestSort} />
                         ...
                         <tbody>
                            {paginatedLoans.map(loan => (
                                <tr key={loan.id} className="border-t border-gray-100">
                                    <td className="p-4 text-gray-600">{formatDate(loan.date)}</td>
                                    <td className="p-4 font-medium text-gray-800">{loan.transactionNo}</td>
                                    <td className="p-4 font-medium text-gray-800">{loan.clientName}</td>
                                    <td className="p-4 text-gray-600">{loan.accountNumber}</td>
                                    <td className="p-4 text-gray-600">{loan.interestPeriod}</td>
                                    <td className="p-4 text-gray-600">{loan.interestRate}%</td>
                                    <td className="p-4 font-medium text-gray-800">{loan.currency} {loan.amount.toLocaleString()}</td>
                                    <td className="p-4"><StatusPill status={loan.status} /></td>
                                    <td className="p-4 text-gray-600">{formatShortDate(loan.deadline)}</td>
                 ...
                 <div className="flex justify-between items-center p-4 border-t border-gray-100 text-sm">
                    <span className="text-gray-600">
                        Data {paginatedLoans.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to {Math.min(currentPage * itemsPerPage, processedLoans.length)} of {processedLoans.length}
                    </span>
